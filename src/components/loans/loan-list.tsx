...
import { fetchLoansWithBooks, markLoanReturned, type LoanWithBook } from "@/lib/data/loans";
...
export function LoanList() {
  ...
  async function markReturned(loan: LoanWithBook) {
    setLoans((prev) =>
      prev
        ? prev.map((l) => (l.id === loan.id ? { ...l, returned_at: new Date().toISOString() } : l))
        : prev,
    );
    try {
      ...
      <Tabs defaultValue="active">
        ...
        <TabsContent value="history" className="flex flex-col gap-3 pt-3">
          ...
          {loans && history.length === 0 && (
            <EmptyState message="Aucun prêt rendu pour l'instant." />
          )}
          {history.map((loan) => (
      ...
      <Link href={loan.books ? `/books/${loan.books.id}` : "#"} className="min-w-0 flex-1">
        ...
        {loan.expected_return_date && !loan.returned_at && (
          <p className="text-xs text-loan">
            Retour prévu le {formatDate(loan.expected_return_date)}
          </p>
        )}
        {loan.returned_at && (
